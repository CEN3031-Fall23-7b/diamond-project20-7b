  ...
  const Replay = () => {
    ...
    useEffect(() => {
      const getReplay = async () => {
        console.log(saveID);
        const activity = await getActivity(saveID);
        setReplay(activity.data.replay.current);
        console.log(activity.data.replay.current);
        dispatchTimelineReducer({
          type: 'FetchReplayLength',
          value: activity.data.replay.current ? activity.data.replay.current.length : 0,
        });
        //set log
        let data = activity.data.replay.current.map((item, index) => {
          return {
            key: index,
